...
if( getTrackingId() == null ) {
  ...
  var userTracking = window.userTracking = ua('UA-51705620-2', getTrackingId());
}
...
if( typeof __isNewInstall != 'undefined' && __isNewInstall == true )  {
  userTracking.event('App Install', getOperatingSystem(), Settings.get('version')).send();
}
else if( typeof __isUpgradeInstall != 'undefined' && __isUpgradeInstall == true )  {
  userTracking.event('App Upgrade', getOperatingSystem(), Settings.get('version')).send();
}
...
if( (typeof __isNewInstall != 'undefined' && __isNewInstall == true) || 
  ...
  userTracking.event('Device Stats', 'Version', Settings.get('version') ).send();
